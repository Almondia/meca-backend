...
[[CardHistory-카테고리ID-기반-카드-히스토리-페이징-조회]]
=== CardHistory 카테고리ID 기반 카드 히스토리 페이징 조회

operation::find-card-history-by-category-id-test/should-return200-when-success-test[snippets='http-request,path-parameters,request-parameters,http-response,response-fields']
